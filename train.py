...
def get_parser():
    ...
    # --- architecture overrides (defaults match get_params so old behavior is unchanged) ---
    parser.add_argument("--embedding_dim", type=int, default=100,
                        help="Token embedding size (was 100).")
    parser.add_argument("--hidden_size1", type=int, default=384,
                        help="BiLSTM hidden size.")
    parser.add_argument("--hidden_size2", type=int, default=384,
                        help="Top LSTM hidden size.")
    parser.add_argument("--dropout", type=float, default=0.5,
                        help="Dropout before the output heads.")
    parser.add_argument("--top_bidirectional", type=str2bool, default=False,
                        help="Make the top LSTM bidirectional.")
    # --- v4 breakthrough arch ---
    parser.add_argument("--arch", type=str, default="base", choices=["base", "v4"],
                        help="'v4' = Conformer-lite + CRF + Focal.")
    parser.add_argument("--use_conformer", type=str2bool, default=True)
    parser.add_argument("--use_crf", type=str2bool, default=True)
    parser.add_argument("--use_focal", type=str2bool, default=True)
    parser.add_argument("--conformer_layers", type=int, default=2)
    parser.add_argument("--attn_window", type=int, default=8)
    parser.add_argument("--punct_weights", type=str, default=None,
                        help="Comma-separated 4 floats: CE class weights for punct [none,comma,period,question].")
    parser.add_argument("--case_weights", type=str, default=None,
                        help="Comma-separated 4 floats: CE class weights for case [lower,upper,cap,mix].")
...
def get_model(params) -> nn.Module:
	...
	if getattr(params, "arch", "base") == "v4":
		from model_v4 import ModelV4
		return ModelV4(params)
...
def run(rank, world_size, args):
	...
	# parse comma-separated class-weight strings into float lists (or None)
	for _k in ("punct_weights", "case_weights"):
		_v = params.get(_k, None)
		if isinstance(_v, str):
			params[_k] = [float(x) for x in _v.split(",")]
	...
	if params.do_finetune:
		...
		# Intervals tuned for continue-training on the FULL dataset
		# (~179 batches/epoch). The original tiny values (5/15/25) were meant
		# for a small adaptation set and validate far too often here.
		params.log_interval = 50
		params.valid_interval = 90
		params.save_every_n = 200
	...
	# Only random-initialize weights when training from scratch.
	# When fine-tuning we must keep the weights loaded from finetune_ckpt,
	# otherwise this would overwrite them with a fresh random init.
	# Skip for v4: kaiming_uniform clobbers the Conformer/attention default
	# init and makes the model collapse to the majority class.
	if not params.do_finetune and getattr(params, "arch", "base") != "v4":
