...
class Model_new(nn.Module):
	...
	def __init__(self, params):
		...
		# Optional bidirectional top LSTM (gives both-side context at the top level).
		self.top_bi = bool(getattr(params, "top_bidirectional", False))
		self.GRU = nn.LSTM(params.hidden_size1*2, params.hidden_size2, 1,
							bidirectional=self.top_bi, batch_first=True)
		# top LSTM output width, then doubled again by the adjacent-token concat
		top_out = params.hidden_size2 * (2 if self.top_bi else 1)
		...
		self.decoder_case = nn.Linear(top_out*2, params.out_size_case)
		self.decoder_punct = nn.Linear(top_out*2, params.out_size_punct)
		self.dropout1 = nn.Dropout(params.dropout)
		self.dropout2 = nn.Dropout(params.dropout)

		# Optional per-class loss weights to counter class imbalance
		# (e.g. rare COMMA/QUESTION). None => plain (unweighted) CrossEntropy.
		# Not persisted in the checkpoint: they only matter during training.
		cw = getattr(params, "case_weights", None)
		pw = getattr(params, "punct_weights", None)
		if cw:
			self.register_buffer("case_w", torch.tensor(cw, dtype=torch.float32), persistent=False)
		else:
			self.case_w = None
		if pw:
			self.register_buffer("punct_w", torch.tensor(pw, dtype=torch.float32), persistent=False)
		else:
			self.punct_w = None
	...
	def forward(
		...
		# pack again so the (optionally bidirectional) top LSTM never reads padding
		biGRU_out = pack_padded_sequence(biGRU_out, lengths=label_lens.cpu(), batch_first=True)
		GRU_out, _ = self.GRU(biGRU_out) # [batch_size, max_label_lens_in_this_batch, (2*)hidden_size2]
		...
		if labels is not None:
			...
			loss_fn_case = nn.CrossEntropyLoss(weight=self.case_w)
			loss_fn_punct = nn.CrossEntropyLoss(weight=self.punct_w)
